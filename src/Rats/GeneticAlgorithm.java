// GENETICALGORITHMS CLASS COURTESY OF PETER ZHOU
...
public class GeneticAlgorithm {
    ...
    public GeneticAlgorithm(int target, int commonWeight, int mutationRate, double minMutateFactor, double maxMutateFactor,
        ...
        for (int i = 0; i < female; i++){
            ...
            //add a female mouse
            lowerbound = commonWeight * ( 1 - FEMALEWEIGHTRANGE);
            upperbound = commonWeight * (1 + FEMALEWEIGHTRANGE) + Double.MIN_VALUE;
            females.add((new Rat("female", Math.random() * lowerbound + (upperbound - lowerbound))));
        ...
        for (int i = 0; i < male; i++){
            ...
            double weight = Math.random() * lowerbound + (upperbound - lowerbound);

            //keep generating
            while (weight < largest){

                weight = Math.random() * lowerbound + (upperbound - lowerbound);
            }
    ...
    public void start(){
        ...
        while (iteration < maxIteration && !reached){
            ...
            for (int i = 0; i < 10; i++){
                ...
                int pickedMale = (int) (Math.random() * 10);
                int pickedFemale = (int) (Math.random() * 10);

                while (chosedMale.contains(pickedMale)){

                    pickedMale = (int) (Math.random() * 10);
                }

                while (chosedFemale.contains(chosedFemale)){

                    pickedFemale = (int) (Math.random() * 10);
                }
                ...
                for (int j = 0; j < litterSize; j++){
                    ...
                    //random.nextDouble(mother.getWeight(), father.getWeight() + Double.MIN_VALUE)
                    double currentWeight = Math.random() * mother.getWeight() + (father.getWeight() - mother.getWeight());
                    boolean willMutate = random.nextInt() % 100 < mutationRate;

                    Rat offspring = new Rat(random.nextInt() % 2 == 0 ? "male" : "female", willMutate ?
                            (Math.random() * minMutateFactor + (maxMutateFactor - minMutateFactor + Double.MIN_VALUE)) * currentWeight : currentWeight);
